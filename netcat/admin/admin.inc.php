...
//    public $headerImage;
     ...
     */
    function ui_config($config_array = array()) {
...
//  if ($dest_subclass['Sub_Class_ID'] == $message['Sub_Class_ID']) { return true; } // Проверка на копирование объекта внутри одного $cc
    ...
    foreach ((array)$multifile_fields as $field) {
        ...
        $settings_http_path_src = nc_standardize_path_to_folder($nc_core->HTTP_FILES_PATH . "/multifile/{$field_id}/");
        $settings_path_src = nc_standardize_path_to_folder($nc_core->DOCUMENT_ROOT . '/' . $nc_core->SUB_FOLDER . '/' . $settings_http_path_src);

        $settings_http_path_dst = nc_standardize_path_to_folder($nc_core->HTTP_FILES_PATH . "/multifile/{$new_field_id}/");
        ...
        foreach ((array)$files as $file) {
            foreach (array('Path', 'Preview') as $path) {
                ...
                if ($file_path) {
                    ...
                    $field_id_part = array_pop($parts);

                    $file_http_path_src = nc_standardize_path_to_folder(implode('/', $parts)) . $field_id_part . '/';
                    $file_path_src = nc_standardize_path_to_folder($nc_core->DOCUMENT_ROOT . '/' . $nc_core->SUB_FOLDER . $file_http_path_src);

                    $file_http_path_dst = nc_standardize_path_to_folder(implode('/', $parts)) . $new_field_id . '/';
                    $file_path_dst = nc_standardize_path_to_folder($nc_core->DOCUMENT_ROOT . '/' . $nc_core->SUB_FOLDER . $file_http_path_dst);

                    $new_file_name = nc_get_filename_for_original_fs($file_name, $file_path_dst);

                    @copy($file_path_src . $file_name, $file_path_dst . $new_file_name);

                    $file[$path] = $file_http_path_dst . $new_file_name;
                }
            }
...
function nc_unique_message_keyword($keyword, $class_id) {
