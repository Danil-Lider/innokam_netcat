...
<?php
    ...
    function ClassForm($ClassID, $action, $phase, $type, $BaseClassID) {
        ...
        ?><input type='hidden' name='IsMultipurpose' value='<?= (int)$data->IsMultipurpose; ?>'><?
        ...
        <table border='0' cellpadding='0' cellspacing='0' width='99%'>
            ...
            <tr>
                <td colspan='2' style='border: none;'>
                    <?php nc_print_component_disable_block_markup_field($data->DisableBlockMarkup); ?>
                </td>
            </tr>
            <tr>
                <td colspan='2' style='border: none;'>
            ...
            <tr>
                <td colspan="2">
                    <? if (!$data->Class_ID || $nc_core->component->can_add_block_markup($data->Class_ID)): ?>
...
##############################################
    ...
    function ActionClassComleted($type, $parentID = false) {
        ...
        $bool_fields = array(
            ...
            'IsAuxiliary', 'DisableBlockMarkup', 'IsOptimizedForMultipleMode', 'IsMultipurpose'
        );
        ...
        if (isset($params['DisableBlockMarkup']) && strlen($params['DisableBlockMarkup'])) {
            $input['DisableBlockMarkup'] = (int)$params['DisableBlockMarkup'];
        }

        if (isset($params['IsMultipurpose']) && strlen($params['IsMultipurpose'])) {
            $input['IsMultipurpose'] = (int)$params['IsMultipurpose'];
        }
...
##############################################
        ...
        ?>
            <div id='maininfoOn'>
                ...
                if (!($row->ClassTemplate || $phase == 15 || $phase == 17)) {
                    ...
                    nc_print_component_disable_block_markup_field($row->DisableBlockMarkup);
                } else {
                    echo "<input type='hidden' name='Class_Group' value='" . CONTROL_CLASS_CLASS_TEMPLATE_GROUP . "'>";
                }
                ...
                <? nc_print_component_multipurpose_fields($row->IsMultipurpose, $row->CompatibleFields); ?>
                <script type="text/javascript">
  ...
  * @param int|bool $is_markup_disabled
  */
function nc_print_component_disable_block_markup_field($is_markup_disabled) {
    ?>
    <div class='nc-form-checkbox-block'>
        <label>
            <input type='checkbox' name='DisableBlockMarkup' value='1'
            <?= ($is_markup_disabled ? 'checked' : ''); ?>>
