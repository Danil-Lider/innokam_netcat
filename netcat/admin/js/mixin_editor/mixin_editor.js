...
(function($) {
    ...
    var nc_mixin_settings_editor = window.nc_mixin_settings_editor = function(options) {
        ...
        this.component_template_id = options.component_template_id;
        this.infoblock_id = options.infoblock_id;
        this.set_breakpoint_type_from_select();
        ...
        this.calculated_colors = {};

        // устанавливаем настройки миксинов из скрытого поля
        try {
            var json = this.find('.nc-mixins-json').val() || '{}';
            this.set_own_settings(JSON.parse(json) || {});
        } catch (e) {}

        // для выбранного пресета выбрана опция в селекте; устанавливаем настройки пресета (если они там есть)
    ...
    nc_mixin_settings_editor.prototype = {
        ...
        set_mixin_input_values: function(mixin_settings_container, settings, trigger_events) {
            ...
            // для редактора свойств «с замком» нужно установить значение lock_sides до остальных значений
            if ('settings.lock_sides' in settings && !settings['settings.lock_sides']) {
                settings = $.extend({ 'settings.lock_sides': settings['settings.lock_sides'] }, settings);
            }
        ...
        init_lock_sides_button: function(lock_button) {
            var container = lock_button.closest('.nc-mixins-mixin-settings-lock-sides, .nc-mixins-mixin-settings'),
                lock_input = container.find(':hidden[name="mixin.settings.lock_sides"]'),
        ...
        /**
         * Добавляет опции для выбора шрифтов (список шрифтов передан в options.fonts при
        ...
        /**
         * Добавляет в HEAD CSS-файлы шрифтов, указанных в options.fonts
         */
        load_fonts: function() {
            var head = $('head');
