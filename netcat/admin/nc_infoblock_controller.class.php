...
class nc_infoblock_controller extends nc_ui_controller {
     ...
     *
     */
    protected function check_area_permissions() {
        /** @var Permission $perm */
        global $perm;
        // @todo CHECK AREA PERMISSIONS PROPERLY
    ...
    protected function place_infoblock($moved_infoblock_id, $position, $other_infoblock_id) {
        ...
        $nc_core = nc_core::get_object();
        $area_keyword = $nc_core->sub_class->get_by_id($moved_infoblock_id, 'AreaKeyword');
        ...
        $parent_container_id = (int)$nc_core->sub_class->get_by_id($moved_infoblock_id, 'Parent_Sub_Class_ID');

        $query_condition = "`Parent_Sub_Class_ID` = $parent_container_id";

        if ($area_keyword) {
            $site_id = $nc_core->sub_class->get_by_id($moved_infoblock_id, 'Catalogue_ID');
            $all_infoblocks = $nc_core->sub_class->get_by_area_keyword($area_keyword, array('site_id' => $site_id), $parent_container_id, true);
            $query_condition .= " AND `AreaKeyword` = '" . $nc_core->db->escape($area_keyword) . "'";
        } else if ($parent_container_id) {
            $all_infoblocks = $nc_core->sub_class->get_by_container_id($parent_container_id);
        } else {
            $all_infoblocks = $nc_core->sub_class->get_by_subdivision_id($subdivision_id, true);
            $query_condition .= " AND `Subdivision_ID` = $subdivision_id";
        }
    ...
    protected function wrap_with_flexbox($infoblock_id, $main_axis) {
        ...
        $container_id = $nc_core->sub_class->create(0, $location + array(
            ...
            'Checked' => 1,
            'Mixin_Settings' => nc_array_json($mixin_settings),
        ));
    ...
    protected function action_show_settings_dialog() {
        ...
        $mixin_presets = $this->get_mixin_preset_options($infoblock_data);
    ...
    /**
     * Возвращает массив с опциями для селекта пресета миксинов:
     * использовать пресет шаблона по умолчанию + не использовать дефолтный пресет + все подходящие пресеты
     * @param array $infoblock_data
     * @return array
     */
    protected function get_mixin_preset_options(array $infoblock_data) {
        $nc_core = nc_core::get_object();
        $db = $nc_core->db;

        $current_preset_id = (int)$infoblock_data['Mixin_Preset_ID'];
        ...
        if ($infoblock_data['Class_ID']) { // это не контейнер: шаблон или компонент может иметь дефолтный пресет
            ...
            $own_preset_id = $db->get_var("SELECT `Mixin_Preset_ID` FROM `Sub_Class` WHERE `Sub_Class_ID` = $infoblock_data[Sub_Class_ID]");

            $component_template_id = $infoblock_data['Class_Template_ID'] ?: $infoblock_data['Class_ID'];
            $default_preset_id = $nc_core->component->get_by_id($component_template_id, 'Mixin_Preset_ID');
            ...
            $own_preset_id = $infoblock_data['Mixin_Preset_ID'];
        }
    ...
    protected function action_paste() {
        ...
        if ($position === 'wrap_before' || $position === 'wrap_after') {
            ...
            $position = str_replace('wrap_', '', $position);
        }
        ...
        if ($paste_mode === 'cut') {
            ...
            $updated_infoblock_id = $pasted_infoblock_id;
            $nc_core->sub_class->move($updated_infoblock_id, $destination);
        } else { // paste_mode = copy
            $updated_infoblock_id = $nc_core->sub_class->duplicate($pasted_infoblock_id, $destination);
        }

        $this->place_infoblock($updated_infoblock_id, $position, $reference_infoblock_id);
    ...
    protected function action_show_mixin_editor() {
        $nc_core = nc_core::get_object();
