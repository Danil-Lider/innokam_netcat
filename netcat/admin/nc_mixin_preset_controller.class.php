...
class nc_mixin_preset_controller extends nc_ui_controller {
    ...
    protected function action_show_edit_dialog() {
        ...
        $is_default_for_current_component_template =
            ...
            $mixin_preset_id &&
            (int)$nc_core->component->get_by_id($current_component_template_id, 'Mixin_Preset_ID') === $mixin_preset_id;
    ...
    protected function action_show_delete_dialog() {
        ...
        $mixin_preset_name = $this->get_table()->where_id($mixin_preset_id)->get_value('Mixin_Preset_Name');
        $number_of_blocks = $db->get_var("SELECT COUNT(*) FROM `Sub_Class` WHERE `Mixin_Preset_ID` = $mixin_preset_id");
        $number_of_templates = $db->get_var("SELECT COUNT(*) FROM `Class` WHERE `Mixin_Preset_ID` = $mixin_preset_id");

        return $this->view('mixin/preset_delete_dialog', array(
            'mixin_preset_id' => $mixin_preset_id,
            'mixin_preset_name' => $mixin_preset_name,
    ...
    protected function action_save() {
        ...
        if ($component_template_id && $set_as_default !== null) {
            nc_db_table::make('Class')->where_id($component_template_id)->update(array(
                'Mixin_Preset_ID' => $set_as_default ? $preset_id : null,
            ));
        }
    ...
    protected function action_delete() {
        ...
        $nc_core->db->query("UPDATE `Class`     SET `Mixin_Preset_ID` = NULL WHERE `Mixin_Preset_ID` = $mixin_preset_id");
        $nc_core->db->query("UPDATE `Sub_Class` SET `Mixin_Preset_ID` = -1   WHERE `Mixin_Preset_ID` = $mixin_preset_id");
