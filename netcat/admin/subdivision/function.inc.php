                ...
                <table class='nc-table nc--wide'>
                        ...
                        <tr>
                            ...
                            <td><?= $children = ChildrenNumber($SubdivisionID) ?>
                                ...
                                <a href="index.php?phase=2&ParentSubID=<? print $SubdivisionID; ?>"><?= CONTROL_CONTENT_SUBDIVISION_FUNCS_ADD ?></a>)
                            </td>
                        </tr>
                        <tr>
            ...
            if ($users) {
                foreach ($users as $user) {
                    if ($user['PermissionSet'] & MASK_READ) {
                        $read_access = true;
                        echo "<i class='nc-icon nc--user-group'></i> ".($is_supervisor ? "<a href='".$ADMIN_PATH."user/index.php?phase=4&UserID=".$user['User_ID']."'>".$user['Login']."</a>" : $user['Login'])." \n";
                    }
                }
            }
            if (!$read_access) {
                ...
                if ($users) {
                    foreach ($users as $user) {
                        if ($user['PermissionSet'] & MASK_COMMENT) {
                            $read_access = true;
                            echo "<li><i class='nc-icon nc--user-group'></i> ".($is_supervisor ? "<a href='".$ADMIN_PATH."user/index.php?phase=4&UserID=".$user['User_ID']."'>".$user['Login']."</a>" : $user['Login'])." \n";
                        }
                    }
                }
                if (!$read_access) {
...
function nc_subdivision_form_edit($sub_env) {
    ...
    $field_main->add(nc_admin_label_color_field($sub_env['LabelColor']));
...
function nc_subdivision_form_mixin_settings($mixin_settings = '{}') {
    $nc_core = nc_core::get_object();
    $editor_data = array(
        'editor_mode' => 'subdivision',
        'settings_field_name' => 'MainArea_Mixin_Settings',
        'data' => array(
            'Mixin_Settings' => $mixin_settings,
        ),
    );
