...
#######################################################################
    ...
    function  ActionForm($SubClassID, $phase, $type) {
        ...
        if ($type == 2) {
            $fieldsets->new_fieldset('mixin', NETCAT_MIXIN_TITLE, NETCAT_MIXIN_TITLE)->add(nc_sub_class_nc_mixins());
        }
...
function ActionSubClassCompleted($type) {
    ...
    if ($type == 2) {
        ...
        $update.= "`Mixin_Preset_ID` = '" . (int)$post_data['Mixin_Preset_ID'] . "', ";
        $update.= "`Mixin_Settings` = '" . $db->escape($post_data['Mixin_Settings']) . "', ";
        $update.= "`Mixin_BreakpointType` = '" . $db->escape($post_data['Mixin_BreakpointType']) . "', ";
    ...
    if (!empty($classTemplatesArr)) {
        $classInfo .= "
                <div>
                    ...
                    <div>
                        <select name='Class_Template_ID' id='Class_Template_ID' onchange=\"if (this.options[this.selectedIndex].value) {loadClassCustomSettings(this.options[this.selectedIndex].value, " . (int)$Array['Sub_Class_ID'] . "); document.getElementById('classtemplateEditLink').disabled = (this.options[this.selectedIndex].value==" . $Array['Class_ID'] . " ? true : false)}\">
