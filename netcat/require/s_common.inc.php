...
function nc_file_field($field_name, $style = "", $classID = "", $caption = false) {
    ...
    if ($sysTable == 2 && $field_name === 'ncSMO_Image') {
        $old = $nc_core->subdivision->get_by_id($message, 'ncSMO_Image');
    }
    ...
    $file_exists = false;
    $file_path = null;
    if ($message && (($systemTableID && $value) || ($action === 'change' && $old) || ($field_name === 'ncSMO_Image' && $old))) {
...
function nc_quickbar_permission() {
    ...
    if ($perm->isAnySubClassAdmin() || in_array($current_sub['Subdivision_ID'], $allow_id)) {
        return true;
    } else {
        return false;
    }
...
function nc_modal_dialog_trigger_init() {
    ...
    $nc(c).off('click'+c).on('click'+c, function() {
        nc.load_dialog($nc(this).data('edit-url'));
    });
}
...
function nc_embed_icon($component_id, $object_id, $field_name, array $tag_attributes = array()) {
    ...
    if ($is_svg) {
        return nc_get_svg_as_icon($nc_core->DOCUMENT_ROOT . $image_path, $tag_attributes);
    } else {
 ...
 */
function nc_get_svg_as_icon($path, array $tag_attributes = array()) {
    if (!isset($tag_attributes['class'])) {
        $tag_attributes['class'] = 'tpl-icon';
    } else if (!preg_match('/\btpl-icon\b/', $tag_attributes['class'])) {
        $tag_attributes['class'] .= ' tpl-icon';
    ...
    $svg = nc_get_svg_for_embedding($path);
 ...
 * @internal не является частью API
 */
function nc_get_svg_for_embedding($path) {
