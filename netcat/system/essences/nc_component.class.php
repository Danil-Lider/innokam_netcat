...
class nc_Component extends nc_Essence {
    ...
    public function add_field($component_id, $properties = array()) {
        ...
        $type = intval($properties['TypeOfData_ID']);
        ...
        $fields = array(
            ...
            'Format' => $format,
            'NotNull' => $not_null,
            'Priority' => intval($db->get_var("SELECT MAX(`Priority`) FROM `Field` WHERE `Class_ID`={$component_id}")) + 1,
            'DoSearch' => $searchable,
        ...
        $sql = "ALTER TABLE `Message{$component_id}` ADD `{$name}` ";
        ...
        if ($default != null && !in_array($type, array(NC_FIELDTYPE_TEXT, NC_FIELDTYPE_FILE, NC_FIELDTYPE_MULTIFILE, NC_FIELDTYPE_DATETIME))) {
        ...
        if ($searchable && !in_array($type, array(NC_FIELDTYPE_TEXT, NC_FIELDTYPE_FILE, NC_FIELDTYPE_MULTIFILE))) {
    ...
    public function add($class_name, $class_group, $params, $class_template = 0, $type = 'useful') {
        ...
        $params_int = array('AllowTags', 'RecordsPerPage', 'NL2BR', 'UseCaptcha', 'CacheForUser', 'IsAuxiliary', 'IsOptimizedForMultipleMode', 'DisableBlockMarkup', 'IsMultipurpose');
        if (!$class_template) {
    ...
    public function update($id, $params = array()) {
        ...
        if (isset($params['IsMultipurpose']) && strlen($params['IsMultipurpose'])) {
            $query[] = "`IsMultipurpose` = " . (int)$params['IsMultipurpose'];
        }
