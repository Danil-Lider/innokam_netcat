...
class nc_file_info {
    ...
    public function get_file_info($component_id, $object_id, $field_name, $include_field_name = true, $prefix_with_f = false, $use_cache = false) {
        ...
        if ($file_path || nc_is_edit_mode()) {
            ...
            $image_path->set_entity(is_numeric($component_id) ? $component_id : strtolower($component_id));
            $image_path->set_field($field_id);
            $image_path->set_object($object_id);
            $image_path->set_file_url($file_info_url);
