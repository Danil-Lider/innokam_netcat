...
class nc_Page extends nc_System {
    ...
    public function get_title() {
        return nc_array_value($this->metatags, 'title') ?: $this->core->subdivision->get_current('Subdivision_Name');
    }
    ...
    protected function clear_browser_partials() {
        ...
        if ($clear_browser_partials) {
            $this->require_asset('nc_partial_load@latest', array('embed' => true));
        }
     ...
     * @param int $priority
     */
    public function add_styles($styles, $priority = self::STYLE_PRIORITY_BLOCK) {
    ...
    public function add_page_mixins($buffer) {
        ...
        foreach (array_reverse($parents) as $parent) {
            ...
            if ($mixin_settings && $mixin_settings !== '{}') {
                ...
                $mixin_properties = array(
                    'Mixin_Preset_ID' => 0,
                    'Mixin_Settings' => $mixin_settings,
                    'Mixin_BreakpointType' => 'viewport',
                );
                nc_tpl_mixin_assembler::assemble(".$mixin_body_css_class", $mixin_list_selector, $mixin_properties, self::STYLE_PRIORITY_BODY);
            }
        }
