...
class nc_partial_object_list extends nc_partial {
    ...
    public function get_content() {
        ...
        $nc_add_list_container_markup = $nc_add_block_markup;
        ...
        if ($nc_add_block_markup) {
            ...
            if (!$inside_admin && $nc_add_list_container_markup && ($cc_env['Mixin_Preset_ID'] || $cc_env['Mixin_Settings'])) {
                nc_tpl_mixin_assembler::assemble(".$nc_mixins_css_class", ".$nc_mixins_list_css_class", $cc_env);
            }
        ...
        if ($admin_mode) {
            ...
            $add_toolbars = !$nc_core->inside_admin && ($isSubClassArray || $container_id);
        ...
        if (
            $nc_area_keyword ||
            $nc_add_list_container_markup // @todo refine conditions
        ) {
            $nc_core->page->require_asset('css_element_queries', array('defer' => false));
        }
