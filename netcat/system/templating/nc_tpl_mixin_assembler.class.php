...
class nc_tpl_mixin_assembler {
     ...
     *      Mixin_Preset_ID
     *      Mixin_Settings
     *      Mixin_BreakpointType
     * @param int $priority
     * @return null|string
     */
    public static function assemble($block_selector, $list_selector, array $block_properties, $priority = nc_page::STYLE_PRIORITY_BLOCK) {
        $mixin_preset_id = $block_properties['Mixin_Preset_ID'];
        $mixin_settings_json = $block_properties['Mixin_Settings'];
        $breakpoint_type = $block_properties['Mixin_BreakpointType'];
        ...
        if ($scripts) {
            $nc_core->page->require_asset('nc_mixin_init', array('embed' => true));
